...
function get_rides_for($user_id)
{
    ...
    $trips_result = mysqli_query($connection, $trips_query);
    $trips = array();
    if ($trips_result) {
        for ($i = 0; $i < mysqli_num_rows($trips_result); ++$i) {
            $row = mysqli_fetch_assoc($trips_result);
...
function update_spots_taken($trip_id)
{
    ...
    $spots_taken = mysqli_num_rows(mysqli_query($connection, $spots_query));
...
function delete_trip($trip_id)
{
    ...
    if ($trip) {
        ...
        $delete_trip = mysqli_query($connection, "DELETE FROM $trip_table WHERE id=$trip_id");
        $delete_place_1 = mysqli_query($connection, "DELETE FROM $place_table WHERE id= $origin ");
        $delete_place_2 = mysqli_query($connection, "DELETE FROM $place_table WHERE id=$destination");
...
/**
 * Deletes all trip requests for a specific trip
 * @param trip_id the id of the trip whose requests should be deleted
 * @return true if successful, false otherwise
 */
function delete_trip_requests($trip_id)
{
    global $connection;
    $trip_request_table = TRIP_REQUEST_TABLE;
    $trip_id = functions\sanitize_string($trip_id);

    $query = "DELETE FROM $trip_request_table WHERE trip_id = $trip_id";
    if (mysqli_query($connection, $query))
        return true;
    return false;
}