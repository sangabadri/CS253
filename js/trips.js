...
jQuery(function () {
  ...
  DrivesController = (function () {
    ...
    DrivesController.prototype.deleteTrip = function (tripId) {
      var _this = this;
      return $.ajax({
        url: '/trips_ajax.php',
        type: 'POST',
        ...
        success: function (response) {
          ...
          if (json.status === 'OK') {
            $('#trip-' + tripId).fadeOut(500, function () {
              $(this).remove();
              if (_this.drives.find('tr').length === 0) {
                _this.statusLoader.fadeOut(500, function () {
                  _this.statusMsg.fadeIn(500);
                });
              }
            });
          } else {
        ...
        error: function (xhr, status) {
          alert('Failed to delete trip: ' + xhr.statusText);
        }
      });
    };
    ...
    DrivesController.prototype.loadDrives = function (drivesData) {
      ...
      this.drives.on('click', '.button-ride-requests', function (e) {
        e.preventDefault();
      ...
      this.drives.on('click', '.button-delete-trip', function (e) {
        e.preventDefault();
        if (confirm('Are you sure you want to delete this trip?')) {
          var tripId = $(this).data('trip-id');
          _this.deleteTrip(tripId);
        }
      });
    };
