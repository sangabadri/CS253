...
function update_ride_request_status($driver_id, $data)
{
    ...
    functions\json_respond(
        'OK',
        'Query performed!',
        array("spots_remaining" => $spots_remaining)
    );
}

function delete_trip($user_id, $data)
{
    $trip = database\get_trip($data['trip_id']);

    if (!$trip)
        return functions\json_respond('ERROR', 'Trip not found!');

    if ($trip['driver_id'] != $user_id)
        return functions\json_respond('ERROR', 'You can only delete trips you are driving!');

    if (!database\delete_trip_requests($data['trip_id']))
        return functions\json_respond('ERROR', 'Failed to delete trip requests!');

    if (database\delete_trip($data['trip_id']))
        return functions\json_respond('OK', 'Trip deleted successfully!');
    else
        return functions\json_respond('ERROR', 'Failed to delete trip!');
}
...
if ($_GET) {
    ...
    else if ($method == 'delete_trip')
        return delete_trip($user_id, $data);
    else
        return functions\json_respond('ERROR', 'Unknown method!');
}